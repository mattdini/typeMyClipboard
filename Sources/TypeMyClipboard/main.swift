...
class MenuBarApp: NSObject {
    ...
    private func updateMenu() {
        ...
        let contentItem = NSMenuItem(title: clipboardContent.isEmpty ? "Clipboard is empty" : "📋 \(clipboardContent)", action: nil, keyEquivalent: "")
...
class TypeMyClipboard {
    ...
    private func getKeyCodeAndModifiers(for character: Character) -> (CGKeyCode, CGEventFlags) {
        ...
        let keyCode = keyMap[Character(lowercaseString)] ?? 0
        ...
        let shiftCharacters: [Character: Bool] = [
        ...
        if shiftCharacters[character] == true {
            modifiers = .maskShift
        }
